...
const HypertensionPredictionForm = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post(`${BASE_URL}/predict-hypertension/`, formData);
      // Displaying the API response message in an alert popup
      alert(response.data.message); // e.g., "You are not at risk of diabetes"
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex items-center justify-center">
      <form onSubmit={handleSubmit} className="bg-white p-6 rounded-lg shadow-lg w-full max-w-md">
        <h2 className="text-2xl font-bold mb-6 text-center">Hypertension Prediction Form</h2>
...
export default HypertensionPredictionForm;
